...
@Component
public class ChangeScenarioAllowedDelayHandler implements CommandHandler {

    private final MetaRequestService metaRequestService;

    public ChangeScenarioAllowedDelayHandler(MetaRequestService metaRequestService) {
    ...
    public Optional<String> handle(String args, Long chatId, Message message) {
        ...
        Optional<String> delay = getArg(args, 1);
        if (delay.isEmpty()) {
            return Optional.of("Предоставьте число, в минутах");
        }
        builder.allowedDelayAfterPing(Integer.parseInt(delay.get()));
